...
# ==========================================
# 3.5: QUERY GENERATION (added for reference as to how the query requests were generated)
# ==========================================
def predict_with_rf_uncertainty(rf_model, X):
    tree_preds = np.array([tree.predict(X) for tree in rf_model.estimators_])
    return tree_preds.mean(axis=0), tree_preds.std(axis=0)

KAPPA_SCHEDULE = [1.5, 0.0]
QUERY_BUDGET = 100
pool_indices = list(range(len(test_df)))
queried_indices = []

# Simulation starts with original train samples
X_base_sim = X_train[:len(train_df)] 
y_base_sim = y_train[:len(train_df)]

# Start the simulation with Round 1 mutants
if os.path.exists("query1_result.csv"):
    q1_mutants = set(pd.read_csv("query1_result.csv")["mutant"].tolist())
    round1_idx = test_df.index[test_df["mutant"].isin(q1_mutants)].tolist()
    queried_indices.extend(round1_idx)
    pool_indices = [idx for idx in pool_indices if idx not in round1_idx]

print("\n Generating Request Files for Rounds 2 & 3...")

for round_num in range(2):
    kappa = KAPPA_SCHEDULE[round_num]
    actual_round = round_num + 2
    
    # Filter queried_indices to only those that actually exist in final_train_df
    valid_queried_idx = []
    current_query_scores = []
    
    for idx in queried_indices:
        mut = test_df.iloc[idx]["mutant"]
        match = final_train_df[final_train_df["mutant"] == mut]
        if not match.empty:
            valid_queried_idx.append(idx)
            current_query_scores.append(match["DMS_score"].values[0])
    
    # Construct consistent training sets for the surrogate
    X_curr = np.vstack([X_base_sim, X_test[valid_queried_idx]])
    y_curr = np.concatenate([y_base_sim, np.array(current_query_scores)])
        
    rf_surrogate = RandomForestRegressor(n_estimators=500, max_features='sqrt', n_jobs=-1, random_state=42+round_num)
    rf_surrogate.fit(X_curr, y_curr)
    
    # Select next mutants
    X_pool = X_test[pool_indices]
    mean_preds, std_preds = predict_with_rf_uncertainty(rf_surrogate, X_pool)
    ucb_scores = mean_preds + (kappa * std_preds)
    
    top_k_idx = np.argsort(ucb_scores)[::-1][:QUERY_BUDGET]
    new_indices = [pool_indices[i] for i in top_k_idx]
    
    # Save the request file
    requested_mutants = test_df.iloc[new_indices]["mutant"].tolist()
    req_filename = f"query{actual_round}_req.txt"
    with open(req_filename, "w") as f:
        f.write("\n".join(requested_mutants))
    
    print(f"Round {actual_round}: Saved {len(requested_mutants)} mutants to '{req_filename}'")
    
    # Move to next round
    queried_indices.extend(new_indices)
